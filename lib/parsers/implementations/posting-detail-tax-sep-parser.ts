...
// 处理 前台入账明细表（区间价税分离）系列表格，因数据量巨大，每个住客每天都会插入一条付款信息，所以要将那些没有付款的无用数据清洗掉。
export class PostingDetailTaxSepParser extends BaseFileParser {

  /**
   * 重写行验证逻辑
   * 目标：过滤掉无意义的流水行（即：消费、税金、净值和付款都为 0 的行）
   */
  protected validateRow(row: any, headers: string[]): boolean {
    ...
    if (!isEffectivelyNonZero(consumeVal) && !isEffectivelyNonZero(taxVal) && !isEffectivelyNonZero(netWorthVal) && !isEffectivelyNonZero(payVal)) {
