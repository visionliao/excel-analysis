...
export function TableAnalysisPanel({ files }: TableAnalysisPanelProps) {
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        <h2 className="text-2xl font-semibold text-foreground">表格分析</h2>
        <p className="text-muted-foreground mt-1">分析已选择的表格数据并导出到数据库</p>
      </div>
      ...
      {files.length > 0 ? (
        <div className="mb-6">
          ...
          <div className="border border-border rounded-lg divide-y divide-border max-h-40 overflow-auto">
            {files.map((file) => (
              <div key={file.id} className="flex items-center gap-3 px-4 py-2">
        ...
        <div className="border border-dashed border-border rounded-lg p-8 text-center mb-6">
          <p className="text-muted-foreground">请先在「表格处理」中选择文件</p>
        </div>
      )}
      ...
      <div className="flex items-center gap-4">
        <Input
          ...
          className="flex-1"
        />
        <Button onClick={handleExportData} disabled={files.length === 0}>
          导出数据
        </Button>
      </div>
    </div>
